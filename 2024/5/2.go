...
func fixUpdates (pages []int, rules [100][]int) []int {
  var pageIndex [100]int

  for i, page := range pages {
    for _, r := range rules[page] {
      if pageIndex[r] > 0 {
        pages[i], pages[pageIndex[r] - 1] = pages[pageIndex[r] - 1], pages[i]
        return fixUpdates(pages, rules)
      }
    }
